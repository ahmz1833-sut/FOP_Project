...
int main(int argc, char const *argv[])
{
    ...
    if(argc == 2 && !strcmp(argv[1], "--install"))
    {
        ...
        if(!CHECK_EXIST(PROGRAM_PATH))
        {
            char result;
            printf("The NeoGIT is not found in PATH for launching commands!\n");
            printf("Do you want to install NeoGIT in a system folder in PATH? (Y/N): ");
            scanf("%1[YNyn]", &result);
            if(toupper(result) == 'Y')
            {
                printf("Installing NeoGIT in " PROGRAM_PATH " ...\n");
